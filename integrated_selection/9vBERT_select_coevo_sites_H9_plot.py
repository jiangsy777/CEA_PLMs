...
def process_subtype_heatmap(subtype_config, csv_path):
    ...
    df['delta_human_prob'] = df['delta_human_prob'].astype(float)
    df = df[df['delta_human_prob'] > 0].reset_index(drop=True)

    df['site1_from1_plosone'] = df['site1_from1_plosone'].astype(int)
    df['site2_from1_plosone'] = df['site2_from1_plosone'].astype(int)

    # 分组聚合+按score升序排序
    df_grouped = df.groupby(['site1_from1_plosone', 'site2_from1_plosone'])['delta_human_prob'].mean().reset_index()
    df_grouped = df_grouped.sort_values(by='delta_human_prob', ascending=True).reset_index(drop=True)
    ...
    # ========== 精确分位数阈值（33.3% 和 66.6%） ==========
    scores = df_grouped['delta_human_prob'].values
    ...
    def assign_level(score):
        if score >= threshold_high:
            ...
            return 2  # low
    df_grouped['level'] = df_grouped['delta_human_prob'].apply(assign_level)
    ...
    for i, ss_cat in enumerate(SS_CATEGORIES):
        for j, x_lab in enumerate(x_labels):
            ...
            if mask.any():
                heatmap_matrix[i, j] = df_grouped.loc[mask, 'delta_human_prob'].values[0]
    ...
    for j, x_lab in enumerate(x_labels):
        ...
        if mask.any():
            ...
            score = df_grouped.loc[mask, 'delta_human_prob'].values[0]
