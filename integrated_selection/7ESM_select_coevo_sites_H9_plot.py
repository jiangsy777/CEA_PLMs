...
def process_subtype_heatmap(subtype_config, csv_path):
    ...
    # df = df[df['delta_human_prob'] != 'single'].reset_index(drop=True)
    # df['site12_mutant_score'] = df['site12_mutant_score'].astype(float)
    # thresh = np.quantile(df['site12_mutant_score'].values, 0.65, method='linear')
    ...
    # 分组聚合+按score升序排序
    df_grouped = df.groupby(['site1_from1_plosone', 'site2_from1_plosone'])['site12_mutant_score'].mean().reset_index()
    df_grouped = df_grouped.sort_values(by='site12_mutant_score', ascending=True).reset_index(drop=True)
    ...
    # ========== 精确分位数阈值（33.3% 和 66.6%） ==========
    scores = df_grouped['site12_mutant_score'].values
    ...
    def assign_level(score):
        if score >= threshold_high:
            ...
            return 2  # low
    df_grouped['level'] = df_grouped['site12_mutant_score'].apply(assign_level)
    ...
    for i, ss_cat in enumerate(SS_CATEGORIES):
        for j, x_lab in enumerate(x_labels):
            ...
            if mask.any():
                heatmap_matrix[i, j] = df_grouped.loc[mask, 'site12_mutant_score'].values[0]
    ...
    for j, x_lab in enumerate(x_labels):
        ...
        if mask.any():
            ...
            score = df_grouped.loc[mask, 'site12_mutant_score'].values[0]
...
if __name__ == "__main__":
    INPUT_CSV = "./generate/HA_H9_avian_PlosOne_ref_site12_AA_pair_top3_site2_for_each_selected_site1_with_esm2_logist_score_with_vBERT_predicted_prob_mean.csv"
    datasets = [
        {
            "title": "H9 Coevolution Score",
            ...
            "ss_ref": '../data/pdb_AlphaFold2/ref_H9.csv',
            "result_path": '.'
        }
    ]
    for ds in datasets:
