...
df5.to_csv('./generate/for_duijizhuzhuangtu_H579_final_selected_mutations_distribution_high_group.csv',index=False)

risk_index_dict = {}
sero_set = ['H1', 'H3', 'H5', 'H7', 'H9']
for sero in sero_set:
    # 1. 统计DF2中当前sero的数目
    sero_count = len(df2[df2['Sero_H'] == sero])
...
# 5. 将风险指数保存到DataFrame
df_risk = pd.DataFrame(list(risk_index_dict.items()), columns=['Sero_H', 'Risk_Index'])
df_risk.to_csv('./generate/risk_index_H13579.csv', index=False)

max_risk = df_risk['Risk_Index'].max()


# 6. 输出结果
print("风险指数计算完成，已保存到 generate/risk_index_H13579.csv")
print(df_risk)
sero_set = ['H1', 'H3', 'H5', 'H7', 'H9']
# 存储所有结果
all_risk_results = []
...
df_risk_individual.to_csv('./generate/risk_index_individual_H13579.csv', index=False)
