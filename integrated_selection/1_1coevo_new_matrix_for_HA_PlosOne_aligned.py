...
RBS_name_lst = [i for i in range(115,133)]+[i for i in range(133,150)]+[i for i in range(188,196)]+[i for i in range(213,230)]
...
for lll in range(len(RBS_name_lst)):
    ...
    df_r1.to_csv('./'+path+'generate/site_coevo_with_site'+str(RBS_name)+'.csv', index=False)
    ...
    #
    # plt.tight_layout()
    # plt.savefig('./'+path+'/fig/RBS'+str(RBS_name)+'/sliding_window'+str(str_len)+'.png',dpi=300)
