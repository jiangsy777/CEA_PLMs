...
amino_acids_lst = list(amino_acids)
path = './'
...
'''
for file in os.listdir(path):
    if file.endswith('.csv') & file.startswith('df_all_after_sampled_to_10w'):
        genename = 'amino_seq_new'
        print (genename)
        df = pd.read_csv(path + file)
        df = shuffle(df, random_state = 42)
        sequences0 = sequences0 + df[genename].tolist()
        
sequences = []
nn = 0
for seq0 in sequences0:
    nn+=1
    print(nn)
    count_aa = 0
    for aa0 in amino_acids_lst:
        count_aa+=seq0.count(aa0)
    if count_aa==len(seq0):
...
training_args = TrainingArguments(
    output_dir="./esm2_finetuned_virus10w_freeze32layers_lr3e-5",  # 模型保存路径
