...
for cds in cds_lst:
    pkl_path = './'+cds+'_sampled_variant_not_in_training_set_ems2_ft_embedding.pkl'
    embed_lst = []
    print(cds+'_finished!')
    df_test = pd.read_csv ('./'+cds+'_sampled_variant_not_in_training_set.csv')
    for i in range(len(df_test)):
        print(i)
        test_sequence1 = str(df_test.loc[i,'seg_RBD'])
